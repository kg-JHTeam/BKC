						...
						<div class="table-responsive">
							...
							<form action="#">
								<div align="right">
									...
									<input class="btn btn-success" type="button"style="padding: 5px" value="메뉴업로드" onclick="location.href='${contextPath}/admin/menuUploadpage.ad'"
									id="insertMenu">
								</div>
								<div class="my-2"></div>
								<!-- 리스트 보여주는 table -->
								<table class="table table-bordered" id="dataTable" width="100%"
									...
									<tbody>
										<c:forEach items="${allList}" var="allList">
											<tr align="center">
												<td>${allList.product_name}</td>
												<td id="menuimgCol"><a href="${allList.path}"
													target="_blank"> <img src="${allList.path}" alt="메뉴사진"
														id="menuimg" class="img-responsive center-block" />
												</a></td>
												<td>${allList.price}원</td>
												<td><c:choose>
                                          <c:when test="${allList.use_status eq true }">
                                             <input class="btn btn-info" type="button" value="판매중"
                                                onclick="javascript:status(${allList.use_status} ,${allList.product_serial})"
                                                id="${allList.product_serial}" />
                                          </c:when>
                                          <c:otherwise>
                                             <input class="btn btn-danger" type="button" value="판매중지" onclick="javascript:status(${allList.use_status} ,${allList.product_serial})" id="${allList.product_serial}" />
                                          </c:otherwise>
                                       </c:choose></td>
												<td><input class="btn btn-primary" type="button" value="수정" onClick="location.href='${contextPath}/admin/menuDetail.ad?product_serial=${allList.product_serial}'" /> 
