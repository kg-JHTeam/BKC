							...
							<form action="#">
								<div align="right">
									<select class="btn btn-info dropdown-toggle" id="select1"
										...
										<option value="chicken" selected>치킨메뉴</option>
								...
								<table class="table table-bordered" id="dataTable" width="100%"
									...
									<tbody>
										<c:forEach items="${adchikenlist}" var="adchikenlist">
											<tr align="center">
												<td>${adchikenlist.product_name}</td>
												<td id="menuimgCol"><a href="${adchikenlist.path}"
													target="_blank"> <img src="${adchikenlist.path}"
														alt="메뉴사진" id="menuimg"
														class="img-responsive center-block" />
														</a></td>
												<td>${adchikenlist.price}원</td>
												<td><c:choose>
														<c:when test="${adchikenlist.use_status eq true }">
															<input class="btn btn-info" type="button" value="판매중" onclick="javascript:status(${adchikenlist.use_status} ,${adchikenlist.product_serial})"id="${adchikenlist.product_serial}" />
														</c:when>
														<c:otherwise>
															<input class="btn btn-danger" type="button" value="판매중지" onclick="javascript:status(${adchikenlist.use_status} ,${adchikenlist.product_serial})" id="${adchikenlist.product_serial}" />
														</c:otherwise>
													</c:choose></td>
												<td><input class="btn btn-primary" type="button"
													value="수정" onClick="location.href='${contextPath}/admin/menuDetail.ad?product_serial=${adchikenlist.product_serial}'" /></td>
											</tr>
										</c:forEach>
