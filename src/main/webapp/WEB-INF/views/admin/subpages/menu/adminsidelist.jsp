							...
							<form action="#">
								<div align="right">
									<select class="btn btn-info dropdown-toggle" id="select1"
										...
										<option value="sidemenu" selected>사이드 메뉴</option>
								...
								<table class="table table-bordered" id="dataTable" width="100%"
									...
									<tbody>
										<c:forEach items="${adsidelist}" var="adsidelist">
											<tr align="center">
												<td>${adsidelist.product_name}</td>
												<td id="menuimgCol"><a href="${adsidelist.path}"
													target="_blank"> <img src="${adsidelist.path}" alt="메뉴사진"
														id="menuimg" class="img-responsive center-block" />
												</a></td>
												<td>${adsidelist.price}원</td>
												<td><c:choose>
														<c:when test="${adsidelist.use_status eq true }">
															<input class="btn btn-info" type="button" value="판매중" onclick="javascript:status(${adsidelist.use_status} ,${adsidelist.product_serial})"id="${adsidelist.product_serial}" />
														</c:when>
														<c:otherwise>
															<input class="btn btn-danger" type="button" value="판매중지"
																onclick="javascript:status(${adsidelist.use_status} ,${adsidelist.product_serial})" id="${adsidelist.product_serial}" />
														</c:otherwise>
													</c:choose></td>
												<td><input class="btn btn-primary" type="button" value="수정"onClick="location.href='${contextPath}/admin/menuDetail.ad?product_serial=${adsidelist.product_serial}'" /></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
